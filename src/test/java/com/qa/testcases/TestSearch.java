	...
	//Search page with Login
		...
		public void searchWithLogin() throws Exception {
			...
			//Login with email
					...
					AssertJUnit.assertTrue("Search results 'shorts' ".trim().equalsIgnoreCase(search.searchResult.getText()));
					Reporter.log("Search result for shorts success with login", true);
					moveToElement(search.searchIcon);
					search.searchBox.clear();
					search.searchBox.sendKeys("jeans");
					search.searchButton.click();
					AssertJUnit.assertTrue("Search results 'jeans' ".trim().equalsIgnoreCase(search.searchResultTwo.getText()));
